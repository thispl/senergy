...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Inventory Type"), "fieldname": "inventory_type", "fieldtype": "Data", "width": 150},
...
def get_conditions(filters):
	...
	if filters.get("country"):
		conditions += " and sle.country = '%s'" % filters.get("country")

	if filters.get("inventory_type"):
		conditions += " and sle.inventory_type = '%s'" % filters.get("inventory_type")	
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			sle.item_code, sle.inventory_type, warehouse, sle.posting_date, sle.actual_qty, sle.valuation_rate,
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql("""
		select
			item.name, item.item_name, item.description, item.item_group, item.inventory_type, item.brand, item.stock_uom %s
		from
			`tabItem` item
			%s
		where
